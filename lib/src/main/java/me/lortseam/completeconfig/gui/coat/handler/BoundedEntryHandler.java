...
public class BoundedEntryHandler<T extends Number> extends BasicEntryHandler<T, BoundedEntry<T>> {
    ...
    public void onUpdate(T value) {
        if (NumberUtils.compare(value, entry.getMin()) < 0) {
            addMessage(new Message(Message.Level.ERROR, new TranslatableText("completeconfig.gui.coat.message.tooSmall", entry.getMin())));
        } else if (NumberUtils.compare(value, entry.getMax()) > 0) {
            addMessage(new Message(Message.Level.ERROR, new TranslatableText("completeconfig.gui.coat.message.tooLarge", entry.getMax())));
        }
